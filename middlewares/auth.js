...
export const protect = async (req, res, next) => {
    ...
    if (
        ...
        try {
            ...
            req.user = m_usuario.findById(decoded.usuario.id).select('-contrasenia');

            // 5. continuar el flujo al siguiente controlador/middleware
            next();

        } catch (error) {
            console.error("Error en el token de Autorizacion : ", error);
            // Unauthorzed
            res.status(401).json({ success: false, message: 'No autorizado, token fallido o expirado. ' });
        }
    }

    //Sino hay token en el header
    if (!token) {
        res.status(401).json({ success: false, message: 'No autorizado, no se encontró token.' });
    }
